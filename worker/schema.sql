...
CREATE TABLE tasks (
  ...
  origin       TEXT NOT NULL DEFAULT 'ai',       -- ai/schedule/goal/app
...
-- ═══════════ 目标(自调度推进循环:到点开 origin='goal' 任务,任务收尾用 sql 写回状态与下次时间)═══════════
CREATE TABLE goals (
  id          INTEGER PRIMARY KEY AUTOINCREMENT,
  title       TEXT NOT NULL DEFAULT '',
  prompt      TEXT NOT NULL DEFAULT '',
...
-- 应用的数据表由 AI 用 sql 工具自建(建议前缀 app_<slug>_,非强制)。
CREATE TABLE apps (
  id          INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE TABLE codes (
  ...
  filename   TEXT NOT NULL,                    -- index.html / index.js / index.css
  content    TEXT NOT NULL DEFAULT '',
