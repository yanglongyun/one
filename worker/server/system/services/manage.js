import * as appRepo from '../../apps/app/repository.js';
...
async function saveApp(db, data) {
    const input = appRepo.normalizeInput(data);
    const app = await appRepo.upsert(db, input);
    const written = await appRepo.writeFiles(db, app.id, input.files);
...
export async function executeManagement(args, hub) {
    ...
    if (action === 'app_save') {
        result = await saveApp(hub.db, data);
        hub.toWeb({ type: 'apps.changed' });
        return result;
    }
    if (action === 'app_delete') {
        if (!data.slug) throw new Error('缺 app slug');
        const ok = await appRepo.remove(hub.db, String(data.slug));
        if (ok) hub.toWeb({ type: 'apps.changed' });
        return { ok };
    }
