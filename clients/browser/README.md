...
1. 从 one 的“添加设备”页面下载最新版扩展 ZIP 并解压，或直接使用本目录源码。
2. 打开 `chrome://extensions`。
3. 打开「开发者模式」。
4. 选择「加载已解压的扩展程序」,加载解压目录或本目录。
5. 点击扩展图标,填写:
   - 主域名:你的 one 地址,例如 `https://one.example.com`
   - 访问密码:和服务端实时通道一致的密码
   - 设备名:用于区分多个浏览器,例如 `chrome`
6. 点击「连接」,角标显示 `on` 后即可使用。
...
| 工具 | 作用 |
|---|---|
| `chrome_debugger` | 通过 `chrome.debugger` 向当前活动标签页透传 CDP 命令,可导航、读 DOM、点击、输入、抓网络等 |
